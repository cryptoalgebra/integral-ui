...
export function ALMTag({ poolAddress }: { poolAddress: Address }) {
    ...
    if (isLoading || !vaults || vaults.length === 0) {
        return (
            <div className="flex h-[26px] w-fit cursor-pointer items-center justify-center rounded-full bg-sky-500/20 border border-sky-800 text-sky-500 px-3 py-1 text-xs font-bold duration-200 hover:opacity-80 max-md:text-xs">
                ALM
            </div>
        );
    }
