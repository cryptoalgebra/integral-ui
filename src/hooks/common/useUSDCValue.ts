...
export function useUSDCPrice(currency: Currency | undefined) {
    ...
    return useMemo(() => {
        ...
        // USDG itself — 1:1 price
        if (TOKENS[chainId].USDG.address.toLowerCase() === currency.wrapped.address.toLowerCase()) {
            return {
                price: new Price(TOKENS[chainId].USDG, TOKENS[chainId].USDG, "1", "1"),
                formatted: 1,
            };
        }
        ...
        if (usdAmount) {
            return {
                price: new Price(currency, TOKENS[chainId].USDG, usdAmount.denominator, usdAmount.numerator),
