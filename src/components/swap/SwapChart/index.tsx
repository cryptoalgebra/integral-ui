...
const getTokenTitle = (chartPair: SwapChartPairType, currencyA: Currency, currencyB: Currency) => {
    switch (chartPair) {
        ...
        case SwapChartPair.A:
            return [<CurrencyLogo currency={currencyA} size={30} />, `${currencyA.symbol}`];
        case SwapChartPair.B:
            return [<CurrencyLogo currency={currencyB} size={30} />, `${currencyB.symbol}`];
    }
};
...
const SwapChart = () => {
    ...
    const [pairImage, pairTitle] = useMemo(() => {
        if (!currencies.INPUT || !currencies.OUTPUT) return [<Loader size={16} color="black" />, "Loading..."];
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between">
                ...
                <div className="flex gap-4 w-fit p-2 bg-card border border-card-border rounded-3xl">
                    <div className="flex gap-2">
                        ...
                        <Button
                            variant={chartSpan === SwapChartSpan.MONTH ? "iconActive" : "icon"}
                            size={"icon"}
                    ...
                    <div className="flex gap-2">
                        ...
                        <HoverCard>
                            <HoverCardTrigger>
                                <Button
                                    variant={chartType === SwapChartView.CANDLES ? "iconActive" : "icon"}
                                    size={"icon"}
                    ...
                    {chartCreated ? (
                        <>
                            <div className="text-3xl font-bold">
                                <span>
                                    {displayValue ? displayValue : currentValue ? currentValue : <Loader size={18} color="black" />}
                                </span>
                                <span className="ml-2">{displayValueCurrency && displayValueCurrency}</span>
                            </div>
                            <div className="text-[#b7b7b7] text-sm">{displayValue ? displayDate : null}</div>
                        </>
                    ) : (
                        <>
