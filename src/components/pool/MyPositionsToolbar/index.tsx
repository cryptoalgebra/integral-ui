...
const MyPositionsToolbar = ({ positionsData, poolId }: MyPositionsToolbar) => {
    ...
    return <div className="flex flex-col md:flex-row items-center justify-between w-full mb-6">
        <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="text-gray-300 font-semibold">{`${positionsData?.length} ${formatPlural(positionsData.length, 'position', 'positions')}`}</div>
            <div className="self-center w-[1px] h-[20px] border border-card-border/40"></div>
            <div className="text-cyan-300 font-semibold">{`${formatUSD.format(myLiquidityUSD || 0)} TVL`}</div>
            <div className="self-center w-[1px] h-[20px] border border-card-border/40"></div>
            <div className="text-green-300 font-semibold">{`${formatUSD.format(myFeesUSD || 0)} Fees`}</div>
        </div>
        <div className="flex w-full md:w-fit mt-4 md:mt-0 gap-4">
            {/* <Button size={'sm'}>Collect all fees</Button> */}
