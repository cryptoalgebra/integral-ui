...
const RemoveLiquidityModal = ({ positionId, enableActions, onSuccess }: RemoveLiquidityModalProps) => {
    ...
    }, [isSuccess, sliderValue, farmingClient, positionId, onSuccess]);

    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                <Button size={"md"} variant={"primaryLink"} className="w-full text-xs">
                    Remove Liquidity
                </Button>
            </DialogTrigger>
            <DialogContent size="md" className="md:min-w-[500px] rounded-xl! bg-card" style={{ borderRadius: "32px" }}>
                <DialogHeader>
